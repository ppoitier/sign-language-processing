...
from slp.config.loading.trainer import (
    ...
    load_recognition_lightning_module,
)
from slp.config.templates.task import (
    SegmentationTaskConfig,
    ContrastiveRecognitionTask,
    RecognitionTaskConfig,
)
...
def load_recognition_task(config: RecognitionTaskConfig):
    task_id = config.id
    data_sets = {}
    data_loaders = {}
    for dataset_name, dataset_config in config.datasets.items():
        dataset, loader = load_recognition_dataset(dataset_config, custom_collate=False)
        data_sets[dataset_name] = dataset
        data_loaders[dataset_name] = loader
    module = load_recognition_lightning_module(
        config, training_dataset=data_sets.get("training")
    )
    return task_id, data_sets, data_loaders, module

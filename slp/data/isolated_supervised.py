...
def _get_wds_mapping_fn(
    ...
    def _map_wds_to_sample(wds_sample):
        ...
        return {
            ...
            "label": label,
            "label_id": (
                int(wds_sample["label.idx"])
                if label_mapping is None
                else label_mapping[sample_id]
            ),
        }
...
class IsolatedSignsRecognitionDataset(Dataset):
    def __init__(
        ...
        self.video_tar_path = video_tar_path
        self.video_ext = video_ext
        self.video_gpu_decoding = video_gpu_decoding

        self.pose_transforms = pose_transforms
        self.video_transform = video_transform

        assert not (
            include_videos and not (video_tar_path and video_tar_index_path)
        ), "You have to specify a video tar path along with the path of its index."
        if video_tar_index_path is not None:
            self.video_tar_index = load_tar_index(video_tar_index_path)
        ...
        web_dataset = wds.DataPipeline(
            ...
            wds.select(
                lambda sample: (
                    (sample["__key__"] in split) if split is not None else True
                )
            ),
        ...
        self.label_to_idx = {
            ...
            for label, label_id in set(
                (sample["label"], sample["label_id"]) for sample in self.samples
            )
        }
    ...
    def load_video(self, sample_id: str) -> Tensor:
        ...
        decoder = VideoDecoder(
            load_bytes_from_tar(self.video_tar_path, offset, size),
            device="cuda" if self.video_gpu_decoding else "cpu",
        )
        video_tensor = decoder.get_frames_in_range(
            start=0, stop=decoder.metadata.num_frames, step=1
        ).data
        return video_tensor
    ...
    def __getitem__(self, idx: int):
        ...
        if self.include_videos:
            sample["video"] = self.load_video(sample["id"])
...
def load_dataset(
    ...
    if config.preprocessing is not None:
        pose_transforms = get_pose_pipeline(
            config.preprocessing.pose_transform_pipeline
        )
        video_transform = get_video_transform_pipeline(
            config.preprocessing.video_transform_pipeline
        )
    ...
    dataset = IsolatedSignsRecognitionDataset(
        ...
        video_tar_path=config.video_tar_path,
        video_tar_index_path=config.video_tar_index_path,

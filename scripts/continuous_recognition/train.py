...
from slp.core.config.experiment import ContinuousRecognitionTaskConfig
from slp.datasets.loading import load_continuous_datasets_and_loaders
from slp.nn.model_builder import build_vac_model
...
@click.option(
    ...
    required=True,
    help="Path to the YAML/JSON continuous recognition training configuration file.",
)
def launch_continuous_recognition_training(config_path):
    config: ContinuousRecognitionTaskConfig = parse_config(config_path, ContinuousRecognitionTaskConfig)
    pprint(config)
    ...
    model = build_vac_model(config.model)
    print(model)
    ...
    x = sample['video']
    x = torch.stack([x for _ in range(2)], dim=0)
    x = x.cuda().float()
    mask = None
    y = model(x, mask)
    print('visual:', y['visual_logits'].shape)
    print('contextual:', y['contextual_logits'].shape)
...
if __name__ == "__main__":
    launch_continuous_recognition_training()
